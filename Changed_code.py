...
class TadpoleDataset(torch.utils.data.Dataset):
    def __init__(self, fold=0, train=True, samples_per_epoch=10, device='cuda', full=False):
        ...
        if split == 'train':
            ...
            self.mask[:10000] = 1  # Set the first 60,000 elements to 1
        else:
            self.mask = torch.zeros(70000, dtype=torch.float32)
            self.mask[60000:62000] = 1  # Set the first 60,000 elements to 1
...
class TadpoleDataset(torch.utils.data.Dataset):
    def __init__(self, fold=0, train=True, samples_per_epoch=10, device='cpu', full=False):
        ...
        if split == 'train':
            self.mask = torch.zeros(70000, dtype=torch.float32)
            self.mask[50000:60000] = 1  # Set the first 60,000 elements to 1
        else:
            self.mask = torch.zeros(70000, dtype=torch.float32)
            self.mask[60000:62000] = 1  
