...
class GCNRW(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        num_nodes =  2708
        values = torch.ones(edge_index.shape[1]).to('cuda')
        A = torch.sparse_coo_tensor(edge_index, values, (num_nodes, num_nodes)).to(torch.float)
        # Ensure A is also on CUDA
        A_hat = A.to('cuda')
        A_hat2 = torch.mm(A_hat, A_hat)
        b = torch.mm(A_hat2, x[0])
        H = torch.sigmoid(-torch.mm(b, self.W))
        H = H.unsqueeze(0)
        return H
